...
Mirror of `client-app/` per spec — only the bundle ID (`in.gymculture.trainer`), the
...
(`in.gymculture.trainer`). Both files are `.gitignore`d.

| File | Source | Drop into |
| -- | -- | -- |
| `google-services.json` | Firebase Console → Android app `in.gymculture.trainer` | `android/app/google-services.json` |
| `GoogleService-Info.plist` | Firebase Console → iOS app `in.gymculture.trainer` | `ios/TrainerApp/GoogleService-Info.plist` (also drag-drop into the Xcode project) |
