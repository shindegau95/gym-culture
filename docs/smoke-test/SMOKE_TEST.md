# Phase 0 — End-to-End Smoke Test (GC-30)
...
- All Sprint 1 + Sprint 2 issues green (GC-17 → GC-29 + GC-62).
- Firebase project `gymculture-prod` provisioned per GC-62 with:
...
export FIREBASE_PROJECT_ID=gymculture-prod
mvn flyway:migrate
mvn spring-boot:run
```

Expect logs:
- `FirebaseApp initialized for project 'gymculture-prod'`
- `Started GymCultureApplication`
...
| -------- | ------------------------ | ---------------------------------------------------------------------------- |
| STAFF    | Admin Web (`localhost:4200`) | Sign in with `staff.test@thegymculture.in` → "Account Pending" card → Firebase Console |
| TRAINER  | Trainer App iOS sim      | Sign in with `trainer.test@thegymculture.in` → `PendingScreen`               |
| CLIENT   | Client App iOS sim       | Sign in with `client.test@thegymculture.in` → `PendingScreen`                |
...
```bash
git tag -a phase-0-complete -m "Phase 0 — Foundation complete (GC-17..GC-30)"
git push origin phase-0-complete
```

Then move GC-30 in Linear from In Review → Done. Sprint 2 is sealed; Phase 1 (Admin Web) is unblocked.
